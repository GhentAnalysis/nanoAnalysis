...
class HistogramVariable(object):
  ...
  def initialize_histogram( self, histname=None ):
    ### initialize a histogram
    if histname is None: histname = self.name
    # case of fixed bin width
    if( self.bins is None ):
      hist = ROOT.TH1D(histname, histname, self.nbins, self.xlow, self.xhigh)
    # case of variable bin width
    else:
      hist = ROOT.TH1D(histname, histname, self.nbins, array.array('d',self.bins) )
    hist.SetDirectory(0)
    hist.Sumw2()
    return hist
